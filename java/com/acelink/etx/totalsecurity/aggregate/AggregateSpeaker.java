...
import com.acelink.etx.totalsecurity.aggregate.codec.AggregateAudioDecoder;
...
public class AggregateSpeaker implements DecodeListener {
  ...
  private final AggregateAudioDecoder audioDecoder;
  /* audio speaker */
  private AudioTrack audioTrack;
  /* decides the audio speaker is mute or not */
  private boolean enabled;

  public AggregateSpeaker(CodecFormat audioFormat) {
    audioDecoder = new AggregateAudioDecoder(audioFormat);
    /* default is playing */
    enabled = true;
  }
  ...
  public void stop() {
    ...
    try {
      if(audioTrack!=null){
        EtxLogger.log(TAG, "stop AAC audio track");
        audioTrack.pause();
        audioTrack.flush();
        audioTrack.stop();
        audioTrack.release();
  ...
  @Override public void onFormatChanged(MediaFormat format) {
    ...
    if (audioTrack==null||audioTrack.getSampleRate() != sampleRate || audioTrack.getChannelCount() != channelCount) {
     ...
     /* Log.i(TAG, "current: ["
          ...
          + "]");*/
      Log.i(TAG, "new: [" + sampleRate + ", " + channelCount + "]");
