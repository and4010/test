...
public class MediaCodecHelper {
    ...
    private static String getAdrenoVersionString(String glRenderer) {
        ...
        FancyLogger.e("MediaCodecHelper","Found Adreno GPU: "+modelNumber);
        return modelNumber;
    }
    ...
    public static void initialize(Context context, String glRenderer) {
        ...
        if (configInfo.reqGlEsVersion != ConfigurationInfo.GL_ES_VERSION_UNDEFINED) {
            FancyLogger.e("MediaCodecHelper","OpenGL ES version: "+configInfo.reqGlEsVersion);
            ...
            if (configInfo.reqGlEsVersion >= 0x30000) {
                FancyLogger.e("MediaCodecHelper","Added omx.nvidia/c2.nvidia to reference frame invalidation support list");
                ...
                FancyLogger.e("MediaCodecHelper","Added omx.qcom/c2.qti to reference frame invalidation support list");
            ...
            if (isGLES31SnapdragonRenderer(glRenderer)) {
                FancyLogger.e("MediaCodecHelper","Added omx.qcom/c2.qti to HEVC decoders based on GLES 3.1+ support");
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N && isPowerVR(glRenderer)) {
                FancyLogger.e("MediaCodecHelper","Added omx.mtk to HEVC decoders based on PowerVR GPU");
                whitelistedHevcDecoders.add("omx.mtk");
                ...
                if (glRenderer.contains("GX6")) {
                    FancyLogger.e("MediaCodecHelper","Added omx.mtk/c2.mtk to RFI list for HEVC");
                    refFrameInvalidationHevcPrefixes.add("omx.mtk");
                    refFrameInvalidationHevcPrefixes.add("c2.mtk");
                }
            }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            try {
                if (decoderInfo.getCapabilitiesForType(mimeType).isFeatureSupported(CodecCapabilities.FEATURE_LowLatency)) {
                    FancyLogger.e("MediaCodecHelper","Low latency decoding mode supported (FEATURE_LowLatency)");
                    return true;
                }
            } catch (Exception e) {
                ...
                for (String supportedOption : testCodec.getSupportedVendorParameters()) {
                    for (String knownLowLatencyOption : knownVendorLowLatencyOptions) {
                        if (supportedOption.equalsIgnoreCase(knownLowLatencyOption)) {
                            FancyLogger.e("MediaCodecHelper",decoderName + " supports known low latency option: " + supportedOption);
                            return true;
                        }
                    }
                }
    ...
    public static boolean decoderSupportsFusedIdrFrame(MediaCodecInfo decoderInfo, String mimeType) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            try {
                ...
                {
                    FancyLogger.e("MediaCodecHelper","Decoder supports fused IDR frames (FEATURE_AdaptivePlayback)");
                    return true;
                }
            } catch (Exception e) {
                // Tolerate buggy codecs
    ...
    public static boolean decoderSupportsAdaptivePlayback(MediaCodecInfo decoderInfo, String mimeType) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (isDecoderInList(blacklistedAdaptivePlaybackPrefixes, decoderInfo.getName())) {
                FancyLogger.e("MediaCodecHelper","Decoder blacklisted for adaptive playback");
                return false;
            }
            ...
            try {
                        ...
                        isFeatureSupported(CodecCapabilities.FEATURE_AdaptivePlayback))
                {
                    ...
                    FancyLogger.e("MediaCodecHelper","Adaptive playback supported (FEATURE_AdaptivePlayback)");
                    return true;
                }
            } catch (Exception e) {
                // Tolerate buggy codecs
    ...
    public static boolean decoderSupportsRefFrameInvalidationHevc(MediaCodecInfo decoderInfo) {
        ...
        if (decoderSupportsAndroidRLowLatency(decoderInfo, "video/hevc") ||
            ...
            FancyLogger.e("MediaCodecHelper","Enabling HEVC RFI based on low latency option support");
            return true;
        }
    ...
    public static boolean decoderSupportsRefFrameInvalidationAv1(MediaCodecInfo decoderInfo) {
        ...
        if (decoderSupportsAndroidRLowLatency(decoderInfo, "video/av01") ||
                decoderSupportsKnownVendorLowLatencyOption(decoderInfo.getName())) {
            FancyLogger.e("MediaCodecHelper","Enabling AV1 RFI based on low latency option support");
            return true;
        }
    ...
    public static boolean decoderIsWhitelistedForHevc(MediaCodecInfo decoderInfo) {
        ...
        if (decoderInfo.getName().contains("sw")) {
            FancyLogger.e("MediaCodecHelper","Disallowing HEVC on software decoder: " + decoderInfo.getName());
            return false;
        }
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.S) {
            FancyLogger.e("MediaCodecHelper","Media performance class: " + Build.VERSION.MEDIA_PERFORMANCE_CLASS);
            if (Build.VERSION.MEDIA_PERFORMANCE_CLASS >= Build.VERSION_CODES.S) {
                FancyLogger.e("MediaCodecHelper","Allowing HEVC based on media performance class");
                return true;
            }
        }
        ...
        if (decoderSupportsAndroidRLowLatency(decoderInfo, "video/hevc")) {
            FancyLogger.e("MediaCodecHelper","Allowing HEVC based on FEATURE_LowLatency support");
            return true;
        }
    ...
    public static boolean isDecoderWhitelistedForAv1(MediaCodecInfo decoderInfo) {
        ...
        //
        if (decoderInfo.getName().contains("sw")) {
            FancyLogger.e("MediaCodecHelper","Disallowing AV1 on software decoder: " + decoderInfo.getName());
            return false;
        }
        ...
        for (String preferredDecoder : preferredDecoders) {
            for (MediaCodecInfo codecInfo : getMediaCodecList()) {
                ...
                if (preferredDecoder.equalsIgnoreCase(codecInfo.getName())) {
                    FancyLogger.e("MediaCodecHelper","Preferred decoder choice is "+codecInfo.getName());
                    return codecInfo;
                }
            }
        }
    ...
    private static boolean isCodecBlacklisted(MediaCodecInfo codecInfo) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            if (!SHOULD_BYPASS_SOFTWARE_BLOCK && codecInfo.isSoftwareOnly()) {
                FancyLogger.e("MediaCodecHelper","Skipping software-only decoder: "+codecInfo.getName());
                return true;
            }
        }
        ...
        if (isDecoderInList(blacklistedDecoderPrefixes, codecInfo.getName())) {
            FancyLogger.e("MediaCodecHelper","Skipping blacklisted decoder: "+codecInfo.getName());
            return true;
        }
        ...
        for (MediaCodecInfo codecInfo : getMediaCodecList()) {
            ...
            for (String mime : codecInfo.getSupportedTypes()) {
                if (mime.equalsIgnoreCase(mimeType)) {
                    ...
                    FancyLogger.e("MediaCodecHelper","First decoder choice is "+codecInfo.getName());
                    return codecInfo;
                }
            }
        }
    ...
    private static MediaCodecInfo findKnownSafeDecoder(String mimeType, int requiredProfile) throws Exception {
        ...
        for (int i = 0; i < 2; i++) {
            for (MediaCodecInfo codecInfo : getMediaCodecList()) {
                ...
                for (String mime : codecInfo.getSupportedTypes()) {
                    if (mime.equalsIgnoreCase(mimeType)) {
                        FancyLogger.e("MediaCodecHelper","Examining decoder capabilities of " + codecInfo.getName() + " (round " + (i + 1) + ")");
                        ...
                        if (i == 0 && !decoderSupportsAndroidRLowLatency(codecInfo, mime)) {
                            FancyLogger.e("MediaCodecHelper","Skipping decoder that lacks FEATURE_LowLatency for round 1");
                            continue;
                        }
                        ...
                        if (requiredProfile != -1) {
                            for (CodecProfileLevel profile : caps.profileLevels) {
                                if (profile.profile == requiredProfile) {
                                    FancyLogger.e("MediaCodecHelper","Decoder " + codecInfo.getName() + " supports required profile");
                                    return codecInfo;
                                }
                            }

                            FancyLogger.e("MediaCodecHelper","Decoder " + codecInfo.getName() + " does NOT support required profile");
                        } else {
                            return codecInfo;
                        }
                    }
    ...
    public static boolean isExynos4Device() {
        try {
            ...
            if (stringContainsIgnoreCase(cpuInfo, "SMDK4")) {
                FancyLogger.e("MediaCodecHelper","Found SMDK4 in /proc/cpuinfo");
                return true;
            }

            // If we see "Exynos 4" also we'll count it
            if (stringContainsIgnoreCase(cpuInfo, "Exynos 4")) {
                FancyLogger.e("MediaCodecHelper","Found Exynos 4 in /proc/cpuinfo");
                return true;
            }
        } catch (Exception e) {
            e.printStackTrace();
            ...
            if (files != null) {
                for (File f : files) {
                    if (stringContainsIgnoreCase(f.getName(), "exynos4")) {
                        FancyLogger.e("MediaCodecHelper","Found exynos4 in /sys/devices/system");
                        return true;
                    }
                }
            }
