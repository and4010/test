    ...
    public fun deFish( bitmap:Bitmap):Bitmap{
      ...
      /*  val colour = bitmap.getPixel(640, 640)
        ...
        */
        ...
        var yS = 0.0
        var y = 0
        Log.e("123"," defish pre cretae time=${(System.currentTimeMillis()-t0)}  " )
...
//        val blue = Color.blue(colour)
        ...
        var yS = 0.0
        var y = dy
        val result = Bitmap.createBitmap(width1R1280, width1R1280,Bitmap.Config.RGB_565)
        val outPixels = IntArray(width1R1280 * width1R1280)
        Log.e("123"," defish cretae time=${(System.currentTimeMillis()-t0)}  " )

        while( y <= dy+width1R1280) {
            var x = dx
            while (x <=dx+width1R1280 ){

                    val  xx= matrix!![x][y]
                    val  yy= matrixY!![x][y]
                    val colour = pixels[(yy.toInt()*BitmapWidth+xx.toInt())]
                    var outIndex=(y-dy)*width1R1280+(x-dx)
                    if (outIndex<outPixels.size) outPixels[(y-dy)*width1R1280+(x-dx)]=colour
        ...
        result.setPixels(outPixels, 0, width1R1280, 0, 0,width1R1280, width1R1280)
        // Log.e("123","i="+i)
